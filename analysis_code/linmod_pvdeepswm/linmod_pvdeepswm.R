...
for (xvar in x_vars){
  for (yvar in y_vars){
    ...
    # Continuous x variables (residualizing for age and sex)
      ...
      # x = residuals(lm(scale(ADB_subset[,xvar]) ~ scale(ADB_subset$Age) + ADB_subset$Sex + scale(ADB_subset$Years_school), na.action=na.exclude))
      # 
      # # Residualize y variables for age and sex
      # y = residuals(lm(scale(ADB_subset[,yvar]) ~ scale(ADB_subset$Age) + ADB_subset$Sex + scale(ADB_subset$Years_school), na.action=na.exclude))
...
# levels(lm_values$y_category) = c("T1w", "T2w", "T1w/T2w", "FLAIR", "qT1", "qT2star", "Volume")
# levels(lm_values$y) = c("T1w_median_WMH", "T1w_median_WMH_deep", "T1w_median_WMH_PV", "T1w_median_WMH_SWM",
#                         "T2w_median_WMH", "T2w_median_WMH_deep", "T2w_median_WMH_PV", "T2w_median_WMH_SWM",
#                         "t1t2ratio_median_WMH", "t1t2ratio_median_WMH_deep", "t1t2ratio_median_WMH_PV", "t1t2ratio_median_WMH_SWM",
#                         "FLAIR_median_WMH", "FLAIR_median_WMH_deep", "FLAIR_median_WMH_PV", "FLAIR_median_WMH_SWM",
#                         "qT1_median_WMH", "qT1_median_WMH_deep", "qT1_median_WMH_PV", "qT1_median_WMH_SWM",
#                         "qT2star_median_WMH", "qT2star_median_WMH_deep", "qT2star_median_WMH_PV", "qT2star_median_WMH_SWM",
#                         "volume_WMH", "volume_WMH_PV", "volume_WMH_SWM", "volume_WMH_deep")
# Graphs for pvals
ggplot(lm_values, aes(x=x, y=y, fill=pval_sig)) +
  ...
  #facet_grid(fct_relevel(y_category, "FLAIR", "T1w", "T2w", "T1w/T2w", "qT1", "qT2star", "Volume")~x_category, scales="free") +
...
# Neurosymposium 2022: Only global results and remove TBV,ICV
lm_values_global = lm_values[!grepl("deep", lm_values$y),]
lm_values_global = lm_values_global[!grepl("PV", lm_values_global$y),]
lm_values_global = lm_values_global[!grepl("SWM", lm_values_global$y),]
lm_values_global = lm_values_global[!grepl("TBV", lm_values_global$x),]
lm_values_global = lm_values_global[!grepl("ICV", lm_values_global$x),]

# Graphs for pvals
ggplot(lm_values_global, aes(x=x, y=y, fill=pval_sig)) +
  geom_tile(color = "white", lwd = 0.1, linetype = 1) +
  scale_fill_gradientn(colors = viridis_pal(direction=-1)(9), limits=c(0, 0.01), breaks=c(0, 0.01), name="P-value") +
  #facet_grid(fct_relevel(y_category, "FLAIR", "T1w", "T2w", "T1w/T2w", "qT1", "qT2star", "Volume")~x_category, scales="free") +
  facet_grid(rows = vars(y_category), cols = vars(x_category), scales="free", space="free") +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1, size = 12),
        ...
        axis.title.x = element_blank(), axis.title.y = element_blank(),
        strip.text = element_text(size=12)) + 
  ggsave("./all_lm_pval_globalWM.jpg", height=10, width=13)


# order_atrophy = c("CT_comp1", "CT_comp2", "CT_comp3", "CT_comp4", "CT_comp5", "CT_comp6", "CT_comp7", "CT_comp8")
# order_cognition = c("AD8", "MoCA_total", "RBANS_attention", "RBANS_delayed_memory", "RBANS_immediate_memory", "RBANS_language", "RBANS_visuospatial_memory", "RBANS_total")
# order_demo = c("Group", "BMI", "Years_school", "APOE4_status", "Alcohol", "Concussion", "Diabetes", "High_BP", "High_chol", "Smoking", "TIA")
# order_x = c(order_atrophy, order_cognition, order_demo)
# levels(lm_values_global) = order_x
# 
# # Redo FDR correction with subset of results
# lm_values_global = lm_values_global[,!grepl("fdr", colnames(lm_values_global))]
# lm_values_global$pval_fdr = p.adjust(lm_values_global$pval, method="fdr")
# lm_values_global$pval_fdr_sig = NA
# lm_values_global$std_beta_fdr_sig = NA
# 
# sig_vars = as.data.frame(matrix(ncol=2))
# colnames(sig_vars) = c("x_num", "y_num")
# 
# for (i in 1:nrow(lm_values_global)){
#   if (lm_values_global$pval_fdr[i] <= 0.10) {
#     lm_values_global$pval_fdr_sig[i] = as.numeric(lm_values_global$pval_fdr[i])
#     lm_values_global$std_beta_fdr_sig[i] = as.numeric(lm_values_global$std_beta[i])
#     
#     sig_vars[nrow(sig_vars)+1,] = c(as.numeric(lm_values_global$x_num[i]), as.numeric(lm_values_global$y_num[i]))
#   }
# }
# sig_vars = sig_vars[-1,]
# 
ggplot(lm_values_global, aes(x=x, y=y)) +
  geom_tile(aes(fill=pval_sig), color = "white", lwd = 0.1, linetype = 1) +
  scale_fill_gradientn(colors = viridis_pal(direction=-1)(9), limits=c(0, 0.01), breaks=c(0, 0.01), name="P-value") +
  facet_grid(rows = vars(y_category), cols = vars(x_category), scales="free", space="free") +
  #geom_rect(data=sig_vars, mapping=aes(xmin=x_num-0.5, xmax=x_num+0.5, ymin=y_num-0.5, ymax=y_num+0.5), size=2, fill=NA, color="red") +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1, size = 15),
        legend.text = element_text(size=15), legend.title = element_text(size=15),
        axis.text.y = element_text(size = 15),
        axis.title.x = element_blank(), axis.title.y = element_blank(),
        strip.text = element_text(size=20), strip.text.y = element_blank()) +
  #scale_x_discrete(labels=c(colnames(ADB_subset[,x_vars]))) +
  #scale_y_discrete(labels=c(colnames(ADB_subset[,y_vars]))) +
  ggsave("./all_lm_pval_globalWM.jpg", height=10, width=13)
