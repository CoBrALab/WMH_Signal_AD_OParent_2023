...
x_vars = seq(2, 31)
x_vars = x_vars[!x_vars %in% c(3, 4, 7)] # remove age and sex
y_vars= seq(32, ncol(ADB_subset))
y_vars = y_vars[!y_vars %in% seq(56, 59)]
...
for (xvar in x_vars){
  for (yvar in y_vars){
    ...
    if (is.factor(ADB_subset[,xvar])==TRUE){
      ...
      # If signal-based measure, add corresponding WMH volume as covariate
      else {
        # If PV, add WMH PV volume as a covariate
        ...
        # If deep, add WMH deep volume as a covariate
        ...
        # If SWM, add WMH SWM volume as a covariate
        ...
        # If global, add WMH global volume as a covariate
        else {
    ...
    # Continuous x variables
      ...
      # If volume, don't residualize out WMH volume
      if (y_categ == "Volume"){
        lin_mod = lm(scale(ADB_subset[,yvar]) ~ scale(ADB_subset[,xvar]) + scale(ADB_subset$Age) + ADB_subset$Sex + scale(ADB_subset$Years_school))
      }
      # If signal-based measure, residualize out WMH volume
      else {
        # If PV, residualize out WMH PV volume
        if (grepl("PV", colnames(ADB_subset)[yvar], fixed=TRUE)==TRUE){
